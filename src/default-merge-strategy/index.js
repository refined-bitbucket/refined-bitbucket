...
export const scriptAlreadyExists = () => Boolean(document.getElementById(SCRIPT_ID));
...
function initAsync(defaultMergeStrategy) {
    return new Promise((resolve, reject) => {
        if (!isPullRequest() || scriptAlreadyExists()) {
            reject('This is not a pull request page, or the default merge script has already been inserted and executed.');
        }
        ...
        const onFulfillPullrequest = async () => {
            ...
            const code = () => {
                ...
                const selectedOption = document.querySelector('option[value=\'defaultMergeStrategy\']');
