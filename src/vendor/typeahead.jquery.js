...
;(function(root, factory) {
    ...
    var highlight = (function(doc) {
        ...
        return function hightlight(o) {
            ...
            function hightlightTextNode(textNode) {
                ...
                var text = _.deburr(textNode.data)
    ...
    var Dataset = (function() {
        ...
        function Dataset(o, www) {
            ...
            this.limit = typeof o.limit === 'undefined' ? 5 : o.limit
                ...
                function sync(suggestions) {
                    ...
                    suggestions =
                        that.limit === null
                            ...
                            : (suggestions || []).slice(
                                  0,
                                  Math.max(1, that.limit)
                              )
                    rendered = suggestions.length
                    that._overwrite(query, suggestions)
                    if (rendered < (that.limit || 1) && that.async) {
                        that.trigger('asyncRequested', query)
                    }
                }
                function async(suggestions) {
                    suggestions = suggestions || []
                    if (!canceled && rendered < (that.limit || 1)) {
                        that.cancel = $.noop
                        that._append(
                            query,
                            that.limit === null
                                ...
                                : (suggestions || []).slice(
                                      0,
                                      Math.max(1, that.limit) - rendered
                                  )
                        )
                        rendered += suggestions.length
        ...
        _.mixin(Typeahead.prototype, {
            ...
            _updateHint: function updateHint() {
                ...
                if (
                    ...
                    frontMatchRegEx.test(_.deburr(data.val)) &&
                        this.input.setHint(val + data.val.substr(val.length))
                } else {
