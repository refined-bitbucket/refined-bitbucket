...
export function init(node, ignorePaths) {
    ...
    filesToRemove.forEach(async li => {
        ...
        const diff = await elementReady(`section[data-identifier="${dataIdentifier}"]`, {target: node});
        diff.remove();
    });

    // Update the "Files changed" summary header to reflect the removed diffs count
    const summaryHeader = node.querySelector('#pullrequest-diff > section.main > h1');
