...
[prismjs-languages]: https://prismjs.com/download.html?#themes=prism&languages=markup+css+clike+javascript+abap+actionscript+ada+apacheconf+apex+apl+applescript+arduino+asciidoc+aspnet+asm6502+autohotkey+autoit+bash+basic+batch+bison+brainfuck+bro+c+csharp+cpp+coffeescript+csp+crystal+css--templating+matlab+mel+mizar+monkey+n4js+nasm+nginx+nim+nix+nsis+objectivec+ocaml+opencl+oz+parigp+parser+pascal+perl+php+php-
